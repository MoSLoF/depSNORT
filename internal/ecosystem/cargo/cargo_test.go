...
// TestIsolatedAllSourcedLockInfersRoot pins the fix for the cargoRoots
// entries[0] fallback. A Cargo.lock scanned with NO source-less crate — a lone
// lock split from the workspace member that would otherwise anchor it, the
// common CI-gate shape — must still infer the real root from topology. Before
// the fix, len(localIDs)==0 anchored the root on the first file-order entry (a
// leaf), collapsing every crate to depth 0 / direct:false and corrupting any
// check keyed on depth, direct-ness, or root identity. The edges are correct
// either way, so per-node checks still fire; only the laddering breaks — which
// is exactly what this test guards.
func TestIsolatedAllSourcedLockInfersRoot(t *testing.T) {
	// leaf-utils is listed first (Cargo.lock is alphabetical), so the pre-fix
	// entries[0] fallback would have made it the root.
...
`)
	...
	// realroot has no incoming edge; it is the root, not alphabetically-first
	// leaf-utils.
	if len(g.Roots) != 1 || g.Roots[0] != "pkg:cargo/realroot@1.0.0" {
		t.Fatalf("roots = %v, want [pkg:cargo/realroot@1.0.0]", g.Roots)
	}
	root := g.Get("pkg:cargo/realroot@1.0.0")
	if root == nil {
		t.Fatal("realroot node missing")
	}
	if root.Depth != 0 {
		t.Errorf("realroot depth = %d, want 0", root.Depth)
	}

	leaf := g.Get("pkg:cargo/leaf-utils@0.5.0")
	if leaf == nil {
		t.Fatal("leaf-utils node missing")
	}
	if !leaf.Direct {
		t.Error("leaf-utils should be direct (realroot -> leaf-utils)")
	}
	if leaf.Depth != 1 {
		t.Errorf("leaf-utils depth = %d, want 1", leaf.Depth)
	}
}

// TestFullyCyclicAllSourcedLockKeepsAnAnchor covers the last-resort fallback:
// when no source-less crate exists AND every crate has an incoming edge (a
// genuine cycle), no crate is in-degree-0, so root inference must still yield a
// single anchor rather than an empty root set — otherwise depth assignment has
// nowhere to start.
func TestFullyCyclicAllSourcedLockKeepsAnAnchor(t *testing.T) {
	lock := []byte(`version = 3
...
`)
	...
	if len(g.Roots) != 1 {
		t.Fatalf("roots = %v, want exactly one anchor for a fully cyclic lock", g.Roots)
	}
}
