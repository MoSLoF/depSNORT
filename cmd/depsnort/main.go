...
scan flags:
  ...
  -exclude string          comma-separated dir names to skip under -recursive,
                           on top of the defaults (node_modules, vendor, .git,
                           …). e.g. -exclude testdata for a production-only scan
...
func cmdScan(args []string) int {
	...
	exclude := fs.String("exclude", "", "comma-separated directory names to skip during -recursive discovery, in addition to the defaults (e.g. testdata,fixtures)")
	...
	if *recursive {
		...
		found, err := discoverProjects(path, adapters, extraSkip)
		if err != nil {
